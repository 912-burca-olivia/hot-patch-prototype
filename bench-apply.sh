...
# ---------- IMPORTANT: Pre-warm to ensure the target class is loaded ----------
# This prevents the agent from failing with "Target class not loaded"
# and ensures agent_ms is present (not NaN).
curl -s http://localhost:8080/api/verify >/dev/null 2>&1 || true
# ------------------------------------------------------------------------------
...
# Execute patch
OUT=$(java -cp "$CP" com.hotpatch.tool.PatchApplier "$PATCHED_CLASS" 2>&1) || true
JAVA_RC=$?
...
if [ -z "$AGENT_MS" ]; then
    # Matches: "HTTP 200 from agent: OK 12.345 ms"
    AGENT_MS=$(echo "$OUT" | sed -nE 's/.*OK *([0-9.]+) *ms.*/\1/p' | head -1)
fi

# Success if java exited OK and we have an agent_ms number and no fatal keywords
if [ "$JAVA_RC" -ne 0 ] || [ -z "${AGENT_MS}" ] || [ "${AGENT_MS}" = "NaN" ] \
   || echo "$OUT" | grep -qi "error\|failed\|exception"; then
    SUCCESS="false"
fi
...
# Output CSV row (matches master header)
